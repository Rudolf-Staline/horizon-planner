...
export function EventCard({ event, columnWidth, selected, onSelect, onChange }: Props) {
  ...
  const move = (e: PointerEvent) => {
    ...
    if (g.mode === 'resize') {
      const durationMin = clamp(g.originalDuration + deltaMin, SNAP_MINUTES, 22 * 60 - g.originalStart)
      setPreview({ durationMin })
      return
    }

    const day = clamp(g.originalDay + Math.round(dx / columnWidth), 0, 6)
    const startMin = clamp(snapMinutes(g.originalStart + deltaMin), START_MIN, 22 * 60 - event.durationMin)
  ...
  return (
    <article
      className={`event-card category-${live.category} ${selected ? 'selected' : ''} ${dragging ? 'dragging' : ''}`}
      style={{ top, height, left, width: columnWidth - 8 }}
      onPointerDown={(e) => begin('move', e)}
      onPointerMove={move}
      onPointerUp={end}
      onClick={(e) => { e.stopPropagation(); onSelect() }}
    >
      {dragging && <div className="time-bubble">{formatTime(live.startMin)}</div>}
      <Move className="event-icon" size={15} />
      <div className="event-title">{live.title}</div>
      {live.kind === 'flexible' && <span className="flex-dot" title="Tâche flexible" />}
      <button
